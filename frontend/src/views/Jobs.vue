  ...
  <div>
    <header class="mb-8 flex justify-between items-center">
      <div>
        ...
        <p class="text-sm text-slate-500 mt-1">Manage upcoming and past jobs</p>
      </div>
    ...
    <div v-else-if="!jobs.length" class="bg-white rounded-xl border border-slate-200 shadow-sm p-12 text-center text-slate-500 text-sm">
      No jobs found.
      ...
      <table class="w-full text-left border-collapse min-w-[700px]">
        <thead class="bg-slate-50 text-slate-500 text-xs uppercase font-semibold tracking-wider">
          <tr>
            ...
            <th class="px-6 py-3">Tradesman</th>
        ...
        <tbody class="divide-y divide-slate-100 text-sm">
          <tr v-for="job in jobs" :key="job.id" class="hover:bg-slate-50 transition-colors">
            <td class="px-6 py-4 text-slate-700 whitespace-nowrap">{{ formatDate(job.scheduled_at) }}</td>
            <td class="px-6 py-4 font-medium text-slate-900">{{ job.client_name }}</td>
            <td class="px-6 py-4 text-slate-700">{{ job.job_type }}</td>
            <td class="px-6 py-4 text-slate-500">{{ job.tradesman_name }}</td>
            <td class="px-6 py-4 text-slate-700">{{ fmt(job.estimated_cost_cents) }}</td>
            <td class="px-6 py-4">
              <span :class="statusClass(job.status)" class="px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize">
                {{ job.status.replace('_', '-') }}
              </span>
            </td>
            <td class="px-6 py-4 text-right">
              <div v-if="job.status === 'scheduled'" class="flex gap-2 justify-end">
                <button @click="complete(job)"
    ...
    <div v-if="showForm" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" @click.self="showForm = false">
      <div class="bg-white rounded-2xl p-8 w-full max-w-lg shadow-xl">
        <h3 class="text-lg font-bold text-slate-900 mb-6">{{ editId ? 'Edit job' : 'New job' }}</h3>
        <form @submit.prevent="save" class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Tradesman</label>
              <select v-model="form.tradesman_id" required
                class="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                <option value="">Select tradesman…</option>
                <option v-for="u in tradesmen" :key="u.id" :value="u.id">{{ u.unit_label }}</option>
              </select>
            </div>
          </div>

          <div>
            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Job Type</label>
            <input v-model="form.job_type" required placeholder="e.g. Plumbing, Electrical, HVAC"
              class="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>

          <div>
            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Address <span class="normal-case font-normal text-slate-400">(optional)</span></label>
          ...
          <div>
            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Estimate ($)</label>
            <input v-model.number="estimateDisplay" type="number" min="0" step="0.01" placeholder="0.00"
              class="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
...
async function load() {
  ...
  try {
    ...
    jobs.value = await get('/api/jobs' + qs)
  } catch (err) {
...
async function loadSelectData() {
  const [c, u] = await Promise.all([get('/api/tenants'), get('/api/units')])
  clients.value   = c
  tradesmen.value = u
}
...
async function save() {
  ...
  try {
    const payload = {
      ...form.value,
      estimated_cost_cents: Math.round(estimateDisplay.value * 100),
    }
    if (editId.value) {
      await post(`/api/jobs/${editId.value}`, payload)
    } else {
...
async function complete(job) {
  if (!confirm(`Mark "${job.job_type}" for ${job.client_name} as completed?`)) return
  try {
...
async function cancel(job) {
  if (!confirm(`Cancel this job with ${job.client_name}?`)) return
  try {
...
async function noShow(job) {
  if (!confirm(`Mark ${job.client_name} as a no-show?`)) return
  try {
    await post(`/api/jobs/${job.id}/no-show`, {})
    load()
  } catch (err) {
    alert('Error: ' + err.message)
  }
